...
@app.route('/users/register',methods=['POST'])  # 1 register feature 
def register():
    ...
    if username =='':
        ...
        return resp
    if email =='':
        resp = jsonify({"msg":'email can not be empty'})
        return resp
...
@app.route('/users/login',methods=['POST'])  # 2 login feature 
def login():
    users = mongo.db.users
    email = request.json['email']
...
@app.route('/users/post',methods=['POST']) # 3 add a post feature 
def post():
...
@app.route('/users/posts',methods=['GET']) # 4  retrieve all the posts feature 
def get_posts():
    posts = mongo.db.posts.find()
    resp = dumps(posts)
    return resp

@app.route('/get_post',methods=['POST']) # 5 retrieve my posts feature 
def get_post():
    users = mongo.db.users
...
@app.route('/file/<filename>')  # 6 download the file feature 
def file(filename):
    return mongo.send_file(filename)   

@app.route('/addcomment',methods=['POST']) # 7 add comment feature
def add_comment():
      if request.method == 'POST':
        users = mongo.db.users
        the_user = users.find_one({"email":request.json['email']})
        ...
        # in posts collection we have a field "comments" which is an array,
        # $push operator is to update the array which is already absent in the document
...
@app.route('/plusLike',methods=['POST']) # 9 like button feature add 1 to the like counter of a specific post
def plusLike():
      if request.method == 'POST':
        users = mongo.db.users
        the_user = users.find_one({"email":request.json['email']})
        ...
        i = 0
        j = 0
        while i < len(repeatedUser['like']): # the length of array like []
            if repeatedUser['like'][i]['user_id'] == the_user['_id']:
                repeated ='true'
            i += 1
        if  repeated == 'true':
            posts.update({ '_id': id},{ '$pull': {'like':{"user_id":the_user['_id']}}})
            return jsonify({'msg':'done!' }) # unclick the like
        else:
        ...
        if exist == 'false':
            ...
            resp = jsonify({'msg':'liked User successfully added!' }) # click the like for the first time
        else:
            resp = jsonify({'displaymsg':'you can like or dislike just once on each post'})
        return resp


@app.route('/plusDislike',methods=['POST']) # 10 dislike button feature add 1 to the dislike counter of a specific post
def plusDislike():
    if request.method == 'POST':
      if request.method == 'POST':
        users = mongo.db.users
...
@app.route('/profileUpload',methods=['POST']) # upload a profile image
def imageUpload():
    if request.method == 'POST':
        users = mongo.db.users
...
def update():
    if request.method == 'POST':
        ...
        if request.json['password'] == confirmpassword:
            ...
            # update the username in the comments box
